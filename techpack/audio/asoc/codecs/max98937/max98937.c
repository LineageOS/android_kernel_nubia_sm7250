...
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
#define EAR_RDC_MIN  (255940713)  // 6.35/ 3.33 * (1<<27)
#define EAR_RDC_MAX  (316399149)  // 7.85 / 3.33 * (1<<27)
#define EAR_RDC_DEFAULT (286169931)  // 7.1 / 3.33 * 134217728
...
static int max989xx_calib_save (uint32_t calib_value, int ch)
{
	...
	pfile_app = filp_open(filename_app, O_RDWR | O_TRUNC | O_CREAT, 0666);
	if (!IS_ERR(pfile) || !IS_ERR(pfile_app)) {
		pr_err("%s: save2 %s, calib_value=%d\n",__func__, filename, calib_value);
					     ...
					     loff_t *ppos)
{
	...
	if (ret == 0) {
		...
		if (!rdc_check_valid(impedance_l, MAX98927L)) {
			impedance_l = SPK_MUTE_VALUE;   //calibration failed specail code
			*payload = SPK_MUTE_VALUE;
			pr_err("%s: calib_status = false, check_valid  fail \n", __func__);
			max98927_set_calib_status(false, MAX98927L);
		} else
			max98927_set_calib_status(true, MAX98927L);
	...
	pr_err("%s: calibrate [impedance_l]=%d \n", __func__, impedance_l);
	ret = snprintf(str, PAGE_SIZE, "%d\n", impedance_l);
...
#if 1 //debug chip status registers
{
                        int reg4 = 0, reg5 = 0, reg6 = 0;
                        regmap_read(max98927->regmap[i], 0x04, &reg4);
                        regmap_read(max98927->regmap[i], 0x05, &reg5);
                        regmap_read(max98927->regmap[i], 0x06, &reg6);
                        pr_info("%s: maximPA:0x04=0x%x,0x05=0x%x,0x06=0x%x\n",
                                __func__, reg4, reg5, reg6);
}
#endif
				}
			}
					    ...
					    struct snd_ctl_elem_value *ucontrol)
{
    ...
    if (!max98927->bIsMax98937[MAX98927L]){
		pr_info("%s: register 0x%02X, value 0x%02X\n",
			__func__, MAX98927_AMP_enables, sel);
		...
		pr_info("%s: register 0x%02X, value 0x%02X\n",
			__func__, MAX98937_AMP_enables, sel);
