...
void aw22xxx_build_leds_name(void)
{
	...
	for(i=0; i<AW22XXX_CFG_NUM_MAX; i++)
	{
		if(i == 0){
			...
			sprintf(buf, "aw_cfg_a%d.bin", i-0x50);
		}else if(i>=0x60 && i<=0x68){
			sprintf(buf, "aw_cfg_b%d.bin", i-0x60);
		}else if(i>=0x70 && i<=0x78){
			sprintf(buf, "aw_cfg_c%d.bin", i-0x70);
		}else if(i>=0x80 && i<=0x88){
			sprintf(buf, "aw_cfg_d%d.bin", i-0x80);
		}else if(i>=0x90 && i<=0x98){
			sprintf(buf, "aw_cfg_e%d.bin", i-0x90);
		}else if(i>=0xa0 && i<=0xa8){
			sprintf(buf, "aw_cfg_f%d.bin", i-0xa0);
		}else{
			strcpy(buf, "null");
		}
